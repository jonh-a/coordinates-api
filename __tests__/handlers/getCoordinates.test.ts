...
describe('test getCoordinates.ts', () => {
  test('test coordinates endpoint', async () => {
    const res = await request(app).get('/coordinates?lat=41.8338705&lon=-87.8971134&include=geocoding,weather');
    checkItemsInBody(
      res.body,
      [
